...
Ce que j’entends par là, c’est que le meilleur moyen de profiter de TypeScript, c’est de profiter de son inférence de types, parce qu'on va pas se mentir, personne n'aime maintenir des types, mais également en évitant d'écrire du code spécifique à TypeScript comme par exemple les `enum` ou les `namespace`.
Pour cela, la meilleure manière de produire une application web écrite en TS, c’est de mettre les types robustes au plus bas, ce qui est bien souvent la couche HTTP dans une application web, car il faut valider les entrées et sorties de notre application.

Et pour cela, commençons par la première bibliothèque que j’utilise en ce moment (et depuis quelques temps maintenant, mais ça fait quelques années que je n’ai pas écrit d’article)
...
<Information>Il est possible d’utiliser des dépendances dans les Playground TypeScript, écrit juste l’import, et ça va aller récupérer la dépendance automatiquement pour pouvoir jouer avec directement dedans.</Information>

## Boxed

Une lib que j’utilise un peu moins mais que je trouve bien pratique de temps en temps, qui s’inspire de la programmation fonctionnelle, c’est [Boxed](https://swan-io.github.io/boxed/) par l’équipe de chez Swan.

Elle permet de mettre en place des structures relativement classique pour faire de la programmation fonctionnelle, ce qui me rappelle [mes petites sessions de Rust](https://www.bearstudio.fr/blog/developpement/rust). On y retrouve Result et Option qui sont bien pratiques pour les fonctions les plus basses pour lesquelles on retournerait par exemple `null` ou `undefined`.
...
Cette structure gère également une intéropérabilité avec la prochaine dépendance que je vais te présenter : `ts-pattern` !
...
On a un composant `Select` qui nous permet de gérer [différent type de `select`](https://github.com/BearStudio/start-ui-web/blob/4745ab66edb5c377102ff02292433039e3d33b0b/src/components/Select/index.tsx#L37-L43): `async`, `creatable`, `async-creatable` et `select` classique.
L’avantage de `ts-pattern` ici par rapport à un `switch` classique, c’est encore une fois de ne pas oublier de cas.
...
Niveau API, je trouve ça bien sympa : soit on est en mode data-first ce qui est plus naturel pour les devs, en mode appel de fonction sur les données et traitement en deuxième genre `map(data, () => {})`, soit on est en mode data-last grâce aux fonctions qui acceptent le [currying](https://stackoverflow.com/questions/36314/what-is-currying#36321), ce qui permet de faire plusieurs traitements de suite grâce à la fonction `.pipe()` par exemple.

## Mention spéciale: nuqs

Marre des sites qui ne gardent pas la pagination comme il faut ou ne gardent pas les informations de filtre et recherche quand tu recharge la page ?
La faute au personne qui implémentent les sites avec des états en mémoire et pas dans l'URL parce qu'elles sont ignorantes, ou parce qu'elles trouvent ça galère de gérer le typage et la logique pour le faire. 
...
Si tu veux en savoir plus sur `nuqs`, je t'invite à aller voir [la conférence de son développeur François Best](https://www.youtube.com/watch?v=6hD2SPP1eus).
...
J’espère que cet article t’as plu et dans tous les cas, n'hésite pas à me le faire savoir sur X ou autres réseaux disponible dans le bas de page.
